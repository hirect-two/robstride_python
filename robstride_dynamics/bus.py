...
class RobstrideBus:
    ...
    def __del__(self):
        if self.is_connected:
            try:
                ...
                # Silently ignore errors during cleanup (network may be down)
                # Just try to close the channel handler if it exists
                try:
    ...
    def connect(self, handshake: bool = True) -> None:
        ...
        try:
            ...
            error_msg = str(e)
            if "Network is down" in error_msg or "Failed to transmit" in error_msg or "No such device" in error_msg:
                raise RuntimeError(
                    f"Cannot connect to CAN interface '{self.channel}'. "
                    f"Error: {error_msg}\n"
                    f"   Please check:\n"
                    f"   1. Interface exists: ip link show {self.channel}\n"
                    f"   2. Configure and bring up interface:\n"
                    f"      sudo ip link set {self.channel} type can bitrate {self.bitrate}\n"
                    f"      sudo ip link set {self.channel} up\n"
                    f"   3. You have permissions (may need sudo)"
                ) from e
            raise
    ...
    def disconnect(self, disable_torque: bool = True) -> None:
        ...
        if disable_torque:
            # Try to disable motors, but don't fail if network is down
            disabled_count = 0
            for motor in self.motors:
                try:
                    ...
                    # Network may be down, just log and continue
                    if "Network is down" in str(e) or "Failed to transmit" in str(e):
                        pass  # Silently ignore network errors during cleanup
                    else:
                        print(f"⚠️ Warning: Could not disable {motor}: {e}")
            if disabled_count > 0:
        ...
        try:
            ...
            # Network may already be down
            if "Network is down" not in str(e):
    ...
    def scan_channel(cls, channel: str, start_id: int = 1, end_id: int = 255) -> dict[int, list[int]]:
        ...
        try:
            ...
            error_msg = str(e)
            if "Network is down" in error_msg or "Failed to transmit" in error_msg:
                raise RuntimeError(
                    f"CAN network '{channel}' is down. Please bring up the interface first.\n"
                    f"   Try: sudo ip link set {channel} type can bitrate 1000000 && sudo ip link set {channel} up\n"
                    f"   Or in two steps:\n"
                    f"   1. sudo ip link set {channel} type can bitrate 1000000\n"
                    f"   2. sudo ip link set {channel} up"
                ) from e
            raise
        ...
        try:
            for device_id in tqdm(range(start_id, end_id), desc="Scanning channel"):
                ...
                try:
                    ...
                    # If network goes down during scan, stop scanning
                    if "Network is down" in str(e) or "Failed to transmit" in str(e):
                        raise RuntimeError(
                            f"CAN network '{channel}' went down during scan. "
                            f"Please check the connection and try again."
                        ) from e
                    # Otherwise, just skip this device and continue
                    continue
        finally:
            bus.disconnect()
    ...
    def transmit(
        ...
        assert (device_id >= 1) and (device_id <= 0xFF), "Device ID out of range"  # Allow ID 0 for recovery
    ...
    def ping_by_id(self, device_id: int, timeout: float | None = None):
        ...
        try:
            ...
            # Re-raise network errors so scan_channel can handle them
            if "Network is down" in str(e) or "Failed to transmit" in str(e):
                raise
            raise
    ...
    def disable(self, motor: str):
        ...
        try:
            ...
            # If network is down, we can't disable, but that's okay during cleanup
            if "Network is down" in str(e) or "Failed to transmit" in str(e):
                raise  # Re-raise to let disconnect() handle it gracefully
            else:
                raise  # Re-raise other errors

    def set_zero_position(self, motor: str, wait_for_response: bool = False):
        """
        Set the motor's current position as zero.
        
        Args:
            motor: Motor name
            wait_for_response: If True, wait for status frame response. 
                              If False (default), just send command and return.
                              Use False when motor is disabled.
        """
        device_id = self.motors[motor].id
        # Send SET_ZERO_POSITION command with 8 bytes of zeros as data
        self.transmit(CommunicationType.SET_ZERO_POSITION, self.host_id, device_id, b"\x00\x00\x00\x00\x00\x00\x00\x00")
        # Wait a bit for the command to be processed
        time.sleep(0.1)
        # Only receive status frame if requested and motor is enabled
    ...
    def write_id(self, motor: str, new_id: int):
        ...
        """
        device_id = self.motors[motor].id
        self.transmit(CommunicationType.SET_DEVICE_ID, new_id, device_id)
